...
**Standalone Mode:**
When reviewing PRs that touch iOS code, `/forge:review` can spawn this as a subagent:
```
Task general-purpose("Run /forge:test-xcode for scheme [name]...")
```

**AO Mode:**
In AO mode, run sequentially in the review session:
```bash
# In AO review session
/forge:test-xcode [scheme]
# Results included in review report
```
