...
**CRITICAL:** This skill documents subagent dispatching patterns that spawn internal subagents. In AO mode, this is FORBIDDEN.

### AO Mode Behavior
When `AO_SESSION` environment variable is set:
1. **DO NOT** dispatch internal subagents
2. Instead, use Agent Teams (`TeamCreate`) for parallel execution
3. AO manages the team lifecycle and coordination
4. Alternatively, execute tasks sequentially in the current session
5. For complex parallel work, print an AO spawn plan

### Example: AO Mode Alternative
```bash
# Use Agent Teams instead of subagent dispatch
TeamCreate({
  team_name: "feature-implementation",
  description: "Execute implementation plan"
})
# AO manages parallel agent execution
```

### Standalone Mode (Not for AO)
The subagent dispatching patterns below are for standalone mode only and must NOT be used in AO sessions.
